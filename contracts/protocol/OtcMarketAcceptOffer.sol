...
abstract contract OtcMarketAcceptOffer is OtcMarketCore {
        ...
        returns (MessagingReceipt memory msgReceipt, AcceptOfferReceipt memory acceptOfferReceipt)
    {
        ...
        bytes32 buyer = msg.sender.toBytes32();

        offer.srcAmountSD -= _params.srcAmountSD;
        emit OfferAccepted(_params.offerId, _params.srcAmountSD, buyer);

        (bytes memory payload, bytes memory options) = _buildAcceptOfferMsgAndOptions(buyer, offer.dstEid, _params);
        ...
        Transfer.transferFrom(
            dstTokenAddress,
            msg.sender,
            address(this) /* TODO: change to the treasury address */,
    ...
    function quoteAcceptOffer(
        ...
        // TODO: shall revert?
        Offer memory offer = offers[_params.offerId];

        (bytes memory payload, bytes memory options) = _buildAcceptOfferMsgAndOptions(_buyer, offer.dstEid, _params);
    ...
    function _buildAcceptOfferMsgAndOptions(
        ...
        bytes memory msgPayload = abi.encodePacked(_params.offerId, _params.srcAmountSD, _buyer);
    ...
    ) internal pure virtual returns (bytes32 offerId, uint64 srcAmountSD, bytes32 buyer) {
        offerId = _payload.toB32(0);
        srcAmountSD = _payload.toU64(32);
        buyer = _payload.toB32(40);
    }

    function _receiveOfferAccepted(bytes calldata _msgPayload) internal virtual override {
        (bytes32 offerId, uint64 srcAmountSD, bytes32 buyer) = _decodeOfferAccepted(_msgPayload);

        Offer storage offer = offers[offerId];

        offer.srcAmountSD -= srcAmountSD;
        emit OfferAccepted(offerId, srcAmountSD, buyer);

        escrow.transfer(offer.srcTokenAddress.toAddress(), buyer.toAddress(), srcAmountSD);
    }
}
